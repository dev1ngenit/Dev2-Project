<div class="row g-3">
    ...
    <div class="col-md-4">
        <label class="form-label">Tender Code</label>
        <input type="text" name="tender_code" class="form-control"
               value="{{ old('tender_code', $tender->tender_code ?? '') }}" readonly>
    </div>
    ...
    <div class="col-md-4">
        ...
        <select name="tender_type" class="form-select">
            @foreach(['EOI','RFQ','Consultant','eGP','Enlistment','RFP'] as $type)
                <option value="{{ $type }}" {{ old('tender_type', $tender->tender_type ?? '') == $type ? 'selected' : '' }}>
                    {{ $type }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-4">
        ...
        <select name="submission_day" class="form-select">
            @foreach(['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'] as $day)
                <option value="{{ $day }}" {{ old('submission_day', $tender->submission_day ?? '') == $day ? 'selected' : '' }}>
                    {{ $day }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-8">
        ...
        <select name="action" class="form-select">
            @foreach(['Talk Today','Talk in this Week','Talk Urgently','Talk with Partner','Email Marketing','Start Relation','Important Client','Meet This Week','Meet Next Week','Attend Pre Bid Meet','Meet Tomorrow','Purchase Schedule','Get More Details','Get ToR / SoW','NO'] as $action)
                <option value="{{ $action }}" {{ old('action', $tender->action ?? '') == $action ? 'selected' : '' }}>
                    {{ $action }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <select name="participate" class="form-select">
            @foreach(['Yes','No','May Be','Need Discussion'] as $opt)
                <option value="{{ $opt }}" {{ old('participate', $tender->participate ?? '') == $opt ? 'selected' : '' }}>
                    {{ $opt }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <select name="submitted" class="form-select">
            @foreach(['Yes','No'] as $opt)
                <option value="{{ $opt }}" {{ old('submitted', $tender->submitted ?? '') == $opt ? 'selected' : '' }}>
                    {{ $opt }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <select name="tender_status" class="form-select">
            @foreach(['Live','Fail','Re Tender','Time Over','Technical Eval','Lost','Not Lowest','Probability','Won','Submitted','Participating'] as $status)
                <option value="{{ $status }}" {{ old('tender_status', $tender->tender_status ?? '') == $status ? 'selected' : '' }}>
                    {{ $status }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <select name="purchase" class="form-select">
            @foreach(['Yes','No','May Be'] as $opt)
                <option value="{{ $opt }}" {{ old('purchase', $tender->purchase ?? '') == $opt ? 'selected' : '' }}>
                    {{ $opt }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-4">
        ...
        <select name="mode_of_submission" class="form-select">
            @foreach(['Hardcopy','Email','Online'] as $opt)
                <option value="{{ $opt }}" {{ old('mode_of_submission', $tender->mode_of_submission ?? '') == $opt ? 'selected' : '' }}>
                    {{ $opt }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-4">
        ...
        <input type="text" name="egp_id" class="form-control"
               value="{{ old('egp_id', $tender->egp_id ?? '') }}">
    </div>
